...
| 8 | **Сортировка подсчётом** (Counting Sort) | не сравнением | `O(n + k)` | 75 |
| 9 | **Поразрядная сортировка** (Radix Sort, LSD) | не сравнением | `O(d·(n + k))` | 164 |
| 10 | **Блочная сортировка** (Bucket Sort) | не сравнением | `O(n)` | 347, 451 |
| 11 | **Timsort** (встроенный `sorted()`) | гибрид merge + insertion | `O(n log n)` | 451, 2418 |
...
рекурсию. Именно поэтому он встроен в **Timsort** (см. §11) и в `std::sort` — на
...
> 📎 Merge sort — половина **Timsort** (§11): там он сливает не половины, а
...
показывает приём, который дальше переиспользуется в поразрядной сортировке (§9).
...
Ровно эта процедура и является одним проходом radix sort (§9).
...
## 10. Блочная сортировка (Bucket Sort)
...
## 11. Timsort — то, что внутри `sorted()`
