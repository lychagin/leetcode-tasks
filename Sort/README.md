...
| 5 | **Сортировка слиянием** (Merge Sort) | разделяй и властвуй | `O(n log n)` | 148 |
| 6 | **Разделение двумя указателями** (Partition / Dutch Flag) | разбиение по признаку | `O(n)` | 905, 75 |
| 7 | **Сортировка подсчётом** (Counting Sort) | не сравнением | `O(n + k)` | 75 |
| 8 | **Поразрядная сортировка** (Radix Sort, LSD) | не сравнением | `O(d·(n + k))` | 164 |
| 9 | **Блочная сортировка** (Bucket Sort) | не сравнением | `O(n)` | 347, 451 |
| 10 | **Timsort** (встроенный `sorted()`) | гибрид merge + insertion | `O(n log n)` | 451 |
...
рекурсию. Именно поэтому он встроен в **Timsort** (см. §10) и в `std::sort` — на
...
> 📎 Merge sort — половина **Timsort** (§10): там он сливает не половины, а
> найденные в данных естественные run'ы. Вторая половина — insertion sort (§4)
> для коротких кусков.

---

## 6. Разделение двумя указателями (Partition)
...
показывает приём, который дальше переиспользуется в поразрядной сортировке (§8).
...
Ровно эта процедура и является одним проходом radix sort (§8).
...
## 10. Timsort — то, что внутри `sorted()`
