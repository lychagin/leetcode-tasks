...
| 2 | **Сортировка выбором** (Selection Sort) | выбором | `O(n²)` | 2418 |
| 3 | **Пирамидальная сортировка** (Heap Sort) | на куче | `O(n log n)` | 912 |
| 4 | **Просеивание вниз** (Sift-down / Heapify) | вспомогательная процедура | `O(log n)` | 912 |
| 5 | **Сортировка вставками** (Insertion Sort) | вставками | `O(n²)` | 147 |
| 6 | **Сортировка слиянием** (Merge Sort) | разделяй и властвуй | `O(n log n)` | 148 |
| 7 | **Разделение двумя указателями** (Partition / Dutch Flag) | разбиение по признаку | `O(n)` | 905, 75 |
| 8 | **Быстрая сортировка** (Quick Sort) | разделяй и властвуй | `O(n log n)` в среднем | 912 |
| 9 | **Quickselect** (быстрый выбор) | частичная сортировка | `O(n)` в среднем | 215 |
| 10 | **Сортировка подсчётом** (Counting Sort) | не сравнением | `O(n + k)` | 75 |
| 11 | **Поразрядная сортировка** (Radix Sort, LSD) | не сравнением | `O(d·(n + k))` | 164 |
| 12 | **Блочная сортировка** (Bucket Sort) | не сравнением | `O(n)` | 347, 451 |
| 13 | **Timsort** (встроенный `sorted()`) | гибрид merge + insertion | `O(n log n)` | 451, 2418 |
...
\*\*\* по сравнениям; обменов при этом всего `O(n)` — меньше, чем у любой другой
сортировки в таблице. См. §3.
...
Файл: `e2418_Sort_the_People.py`

**Задача:** отсортировать имена по убыванию роста. Рост уникален, `n ≤ 1000` —
именно поэтому здесь можно позволить себе `O(n²)` и написать сортировку руками.

**Идея:** на каждом шаге ищем **максимум** в неотсортированной части и ставим его в
её начало одним обменом. После `i`-го шага первые `i` элементов уже стоят
окончательно.

```
[ готово | ищем максимум здесь ]
            ↑
     нашли → меняем местами с первым элементом неотсортированной части
```

```python
def selectionSort(self, names, heights):
    n = len(heights)
    names, heights = list(names), list(heights)   # копии: вход не портим

    for i in range(n - 1):
        max_idx = i                               # ищем максимум в хвосте
        for j in range(i + 1, n):
            if heights[j] > heights[max_idx]:
                max_idx = j

        if max_idx != i:                          # ставим его в начало хвоста
...
Здесь сортируем **по убыванию**, поэтому ищем максимум. Для возрастания — ровно тот
же код с `<` вместо `>` (ищем минимум).

Разбор `names = ["Mary", "John", "Emma"]`, `heights = [180, 165, 170]`:

| i | Ищем максимум среди | Нашли | Обмен | heights | names |
|---|---|---|---|---|---|
| 0 | `180, 165, 170` | 180 (idx 0) | не нужен (`max_idx == i`) | `[180, 165, 170]` | `[Mary, John, Emma]` |
| 1 | `165, 170` | 170 (idx 2) | idx1 ↔ idx2 | `[180, 170, 165]` | `[Mary, Emma, John]` ✅ |

**Три детали реализации:**

| Деталь | Зачем |
|---|---|
| `list(names)`, `list(heights)` | функция не портит переданные списки — вызывающий код может ими ещё пользоваться |
| Оба массива меняются **синхронно** | имя и рост — одна сущность, разъедешься в одном обмене → ответ неверный |
| `if max_idx != i` | пропускаем бессмысленный обмен элемента с самим собой |

**Почему цикл до `n - 1`, а не до `n`?** Когда все элементы кроме последнего
расставлены, последний автоматически на своём месте — искать максимум среди одного
элемента незачем.

### Чем отличается от пузырька

Обе сортировки — `O(n²)` по сравнениям, но ведут себя по-разному:

| | Selection Sort | Bubble Sort |
|---|---|---|
| Сравнений | `O(n²)` всегда | `O(n²)`, с флагом — `O(n)` на отсортированном |
| **Обменов** | **ровно `n - 1`** (в худшем случае) | до `O(n²)` |
| Ранний выход | невозможен | есть (флаг `swapped`) |
| На отсортированных данных | всё те же `O(n²)` | `O(n)` |
| Стабильность | нет | да |

Главный козырь selection sort — **минимум обменов**. Это важно, когда перестановка
элемента дорогая: например, элементы большие и копируются целиком, или речь о
записи во флеш-память с ограниченным ресурсом. Сравнения дёшевы, записи — нет.

**Почему selection sort нестабильна.** Обмен «дальнего» максимума с текущей
позицией перебрасывает элемент через всю неотсортированную часть, ломая исходный
порядок равных:

```
[3a, 3b, 1]  →  ищем минимум = 1 (idx 2), меняем с idx 0
[1, 3b, 3a]  ←  3b обогнал 3a, хотя изначально шёл после
```

В задаче 2418 это неважно — рост по условию уникален, равных элементов не бывает.

### Связь с пирамидальной сортировкой

Heap sort (§4) — это **тот же алгоритм**, но с быстрым поиском максимума:

| | Selection Sort | Heap Sort |
|---|---|---|
| Как ищем максимум | линейным проходом, `O(n)` | берём корень кучи, `O(1)` + `O(log n)` на восстановление |
| Итого | `O(n²)` | `O(n log n)` |

То есть куча — это структура данных, которая превращает «найти максимум» из
линейной операции в логарифмическую. Идея алгоритма при этом не меняется:
«извлекай максимум, клади в конец, повторяй».

### Сложность

| | Оценка |
|---|---|
| Время (лучший = средний = худший) | `O(n²)` |
| Обменов | `O(n)` |
| Память | `O(1)` (в версии на месте; здесь `O(n)` из-за копирования входа) |
| Стабильность | нет |

> ⚠️ На [912. Sort an Array](https://leetcode.com/problems/sort-an-array/) selection
> sort получит **TLE** так же, как пузырёк: там `n ≤ 5·10⁴`, то есть 2.5 миллиарда
> сравнений. Задача 2418 с `n ≤ 1000` — как раз тот размер, где `O(n²)` уместна.

### Альтернатива в том же файле

Метод `myFirstSolution` решает задачу идиоматично — через встроенный `sort()`
(Timsort, §13):
...
| | `selectionSort` | `myFirstSolution` |
|---|---|---|
| Время | `O(n²)` | `O(n log n)` |
| Строк кода | ~12 | 3 |
| Зачем нужен | понять алгоритм | так пишут в реальном коде |

`zip` здесь склеивает имя с ростом в пару, чтобы они гарантированно переезжали
вместе — ту же задачу в ручной версии решает синхронный обмен в двух массивах.
...
рекурсию. Именно поэтому он встроен в **Timsort** (см. §13) и в `std::sort` — на
...
Файл: `e148_Sort_List.py`

**Идея:** разделяй и властвуй. Делим список пополам, рекурсивно сортируем каждую
половину, затем **сливаем** две отсортированные части в одну. Слияние двух
отсортированных списков — линейная операция, а глубина рекурсии `log n`, отсюда
`O(n log n)`.
...
Почему именно merge sort для связного списка: у списка нет случайного доступа, из-за
чего quick sort и heap sort работают плохо. А вот слияние списку подходит идеально —
достаточно **перецеплять указатели**, не выделяя новый массив. Это тот редкий
случай, когда merge sort требует `O(1)` дополнительной памяти (не считая `O(log n)`
на стек рекурсии).
...
def sortList(self, head):
    if head is None or head.next is None:   # база: 0 или 1 элемент — уже отсортирован
        return head
...
### Шаг 1: divide — поиск середины через fast/slow

```python
def divide(self, head):
    fast = head.next          # ВАЖНО: fast стартует на шаг впереди
    slow = head
    while fast is not None and fast.next is not None:
        slow = slow.next
        fast = fast.next.next
    mid = slow.next
    slow.next = None          # РАЗРЫВАЕМ список на две независимые части
    return head, mid
```

Классический приём «черепаха и заяц»: `fast` идёт вдвое быстрее, поэтому когда он
добирается до конца, `slow` стоит на середине.

```
[4] → [2] → [1] → [3] → None

старт:      slow=4, fast=2        (fast = head.next)
итерация 1: slow=2, fast=3        (slow.next = 2; fast.next.next = 1.next = 3)
проверка:   fast=3, fast.next=None → цикл кончился

mid = slow.next = 1;  slow.next = None  ← разрыв

получили:  [4] → [2] → None   и   [1] → [3] → None
```

**Две детали, на которых легко ошибиться:**

| Деталь | Что будет, если сделать иначе |
|---|---|
| `fast = head.next`, а не `head` | При длине 2 `slow` останется на первом узле только с этим сдвигом. Со стартом `fast = head` для `[2, 1]` получится `slow = 1`, `mid = None`, левая часть = весь список → **бесконечная рекурсия** |
| `slow.next = None` | Без разрыва обе «половины» останутся одним списком, и рекурсия никогда не дойдёт до базы |

Для нечётной длины левая часть получается на один элемент длиннее: `[1,2,3,4,5]` →
`[1,2,3]` и `[4,5]`. На корректность это не влияет.

### Шаг 2: merge — слияние двух отсортированных списков

```python
def merge(self, list1, list2):
    idx1, idx2 = list1, list2

    result = ListNode(0)         # фиктивная голова — как в задаче 147
    current = result
    ...
    # один из списков кончился — прицепляем остаток второго целиком
    current.next = idx2 if idx1 is None else idx1
    return result.next
```

Разбор слияния `[2, 4]` и `[1, 3]`:

| Шаг | idx1 | idx2 | Сравнение | Берём | Результат |
|---|---|---|---|---|---|
| 1 | 2 | 1 | 2 < 1? нет | из list2: **1** | `1` |
| 2 | 2 | 3 | 2 < 3? да | из list1: **2** | `1 → 2` |
| 3 | 4 | 3 | 4 < 3? нет | из list2: **3** | `1 → 2 → 3` |
| 4 | 4 | None | list2 кончился | остаток list1: **4** | `1 → 2 → 3 → 4` ✅ |

**Зачем «прицепить остаток целиком»?** Оставшийся кусок уже отсортирован и весь
больше любого выданного элемента — перебирать его поэлементно незачем, достаточно
одного присваивания указателя.

> ⚠️ **`<` против `<=` — это стабильность.** При `idx1.val < idx2.val` в случае
> равенства мы берём элемент из **второго** списка, то есть более поздний по
> исходному порядку — сортировка становится нестабильной. Стабильная версия:
> `if idx1.val <= idx2.val`. Для 148 разницы нет (сортируем числа), но при
> сортировке объектов по ключу это критично — и именно поэтому «настоящий»
> merge sort считается стабильным.

### Сложность

| | Оценка | Почему |
|---|---|---|
| Время | `O(n log n)` — и в лучшем, и в худшем | `log n` уровней рекурсии × `O(n)` на слияние каждого уровня |
| Память (список) | `O(log n)` | только стек рекурсии; узлы перецепляются на месте |
| Память (массив) | `O(n)` | нужен буфер под слияние |
| Стабильность | да (с `<=`) | — |

Обратите внимание: у merge sort **нет** плохого случая. В отличие от quick sort,
где неудачный пивот даёт `O(n²)`, здесь разбиение всегда пополам по построению.

### Merge sort на массиве

Для сравнения — тот же алгоритм, но с буфером:
...
| | Список (148) | Массив |
|---|---|---|
| Доп. память | `O(log n)` — стек | `O(n)` — буфер |
| Поиск середины | обход fast/slow, `O(n)` | `len(nums) // 2`, `O(1)` |
| Перемещение данных | перецепка указателей | копирование элементов |

> 📎 Merge sort — половина **Timsort** (§13): там он сливает не половины, а
> найденные в данных естественные run'ы. Вторая половина — insertion sort (§5)
> для коротких кусков.

---

## 7. Разделение двумя указателями (Partition)
...
### 912. Sort an Array — вариант `quick_sort`

Файл: `e912_Bubble_Sort_an_Array.py`

**Идея:** тот же «разделяй и властвуй», что у merge sort (§6), но с обратным
порядком работы.

| | Merge Sort | Quick Sort |
|---|---|---|
| Деление | тривиальное (пополам по индексу) | **тяжёлое** (partition по значению) |
| Склейка | **тяжёлая** (слияние) | тривиальная (ничего не делаем) |
| Где основная работа | после рекурсии | **до** рекурсии |

Выбираем опорный элемент (**pivot**) и переставляем массив так, чтобы слева
оказалось всё меньшее, справа — всё большее. После этого части сортируются
независимо, а склеивать их не нужно — они уже стоят на своих местах.

```
[ ... меньше пивота ... | ... больше пивота ... ]
                        ↑
            граница, найденная partition'ом
```

```python
def quick_sort(self, nums, low, high):
    if low >= high:                       # 0 или 1 элемент — сортировать нечего
        return
    ...
    while left < right:
        while nums[left] < pivot:         # ищем слева «нарушителя» >= pivot
            left += 1
        while nums[right] > pivot:        # ищем справа «нарушителя» <= pivot
            right -= 1

        if left <= right:                 # оба нашлись и не пересеклись — меняем
            nums[left], nums[right] = nums[right], nums[left]
            left += 1
            right -= 1

    self.quick_sort(nums, low, right)     # левая часть
    self.quick_sort(nums, left, high)     # правая часть
```

Это **схема Хоара** со встречными указателями — та же механика, что в задачах 905
и 75 (§7), только признак деления не «чётное/нечётное», а «меньше пивота».

### Разбор `[5, 2, 3, 1]`

Для воспроизводимости возьмём пивот не случайный, а средний элемент:

**Вызов `quick_sort(0, 3)`**, `nums = [5, 2, 3, 1]`, `pivot = 2`

| left | right | Что нашли | Действие | Массив после |
|---|---|---|---|---|
| 0 | 3 | `5 >= 2` и `1 <= 2` | обмен, `left=1`, `right=2` | `[1, 2, 3, 5]` |
| 1 | 1 | `2 >= 2` и `2 <= 2` | обмен с самим собой, `left=2`, `right=0` | `[1, 2, 3, 5]` |
| 2 | 0 | `left > right` | выход из цикла | — |

Рекурсия: `(0, 0)` — база, и `(2, 3)`.

**Вызов `quick_sort(2, 3)`**, кусок `[3, 5]`, `pivot = 3`

| left | right | Что нашли | Действие |
|---|---|---|---|
| 2 | 2 | `3 >= 3` и `3 <= 3` | обмен с самим собой, `left=3`, `right=1` |

Рекурсия: `(2, 1)` и `(3, 3)` — обе базовые. Итог: `[1, 2, 3, 5]` ✅

### Почему указатели после цикла всегда расходятся

Каждый обмен двигает **оба** указателя навстречу друг другу, поэтому после него
либо `left > right`, либо они ещё не встретились. Выйти из цикла с `left == right`
невозможно — значит, `right` всегда строго меньше `left`, и два рекурсивных
диапазона `(low, right)` и `(left, high)` **не пересекаются**.

Важнее другое: оба диапазона **строго меньше** исходного. Первый обмен гарантирован
(пивот — элемент массива, значит хотя бы один элемент `>= pivot` и один `<= pivot`
существуют), а он сдвигает `right` минимум на 1 влево от `high` и `left` минимум на
1 вправо от `low`. Без этого свойства рекурсия зациклилась бы.

> ⚠️ Обратите внимание: `quick_sort` возвращает `None` и сортирует **на месте**.
> `sortArray` вызывает её и возвращает сам `nums` — не перепутайте с merge sort,
> который возвращает новую голову списка.

### Худший случай и почему пивот случайный

```mermaid
flowchart TD
    A["partition"] --> B{"Как разделился массив?"}
    B -->|"примерно пополам"| C["глубина log n<br/>× O(n) на уровень<br/>= O(n log n)"]
    B -->|"1 и n-1"| D["глубина n<br/>× O(n) на уровень<br/>= O(n²)"]
```

Если пивотом всегда брать крайний элемент, то на **уже отсортированном** массиве
каждый пивот окажется минимумом, деление выйдет «1 и остальные», и получится
`O(n²)`. На 912 это гарантированный **TLE** (`n ≤ 5·10⁴` → 2.5 млрд операций) и
вдобавок `RecursionError`: глубина рекурсии `5·10⁴` при лимите Python в 1000.

`random.randint(low, high)` убирает эту проблему: специально подобранного «плохого»
входа не существует, потому что пивот не зависит от данных. Вероятность плохого
деления на каждом шаге мала, а вероятность плохого деления `log n` раз подряд —
исчезающе мала.

Альтернатива без `random` — **медиана из трёх** (первый, средний, последний
элементы): дешевле генератора случайных чисел и тоже убивает отсортированный
случай.

Тесты `test_quick_sort_worst_cases` в `test_e912.py` гоняют ровно эти входы:
отсортированный, обратный, все элементы равны и чередование `[0,1,0,1,...]`.

### Сложность

| | Оценка | Комментарий |
|---|---|---|
| Время (средний) | `O(n log n)` | `log n` уровней × `O(n)` на partition |
| Время (худший) | `O(n²)` | вырожденное деление; со случайным пивотом практически недостижимо |
| Память | `O(log n)` | только стек рекурсии — массив сортируется на месте |
| Стабильность | нет | обмены перебрасывают элементы через весь массив |

**Три сортировки из этого файла рядом:**

| | Bubble | Heap | Quick |
|---|---|---|---|
| Худший случай | `O(n²)` | `O(n log n)` | `O(n²)` |
| Память | `O(1)` | `O(1)` | `O(log n)` |
| На практике | не проходит 912 | проходит | **обычно быстрее heap** |

Quick sort выигрывает у heap sort на реальных данных, несмотря на худшую
гарантию: partition читает память **последовательно**, что отлично ложится на
процессорный кэш, а heap sort прыгает по индексам `2i+1` / `2i+2`.

### Много одинаковых элементов

Обычное разбиение на две зоны плохо работает, когда в массиве много дубликатов:
элементы, равные пивоту, разъезжаются по обеим частям и обрабатываются повторно.

Лечится **трёхпутевым разбиением** на зоны `< pivot`, `== pivot`, `> pivot` —
рекурсия идёт только в первую и третью, а средняя уже готова. Это буквально
алгоритм голландского флага из задачи 75 (§7), только границы задаются сравнением
с пивотом, а не значениями 0/1/2.

---

## 9. Quickselect — быстрый выбор

### 215. Kth Largest Element in an Array

Файл: `e215_Kth_Largest_Element_in_an_Array.py`

**Идея:** quickselect — это quick sort, у которого выбросили половину работы. После
partition мы знаем, по какую сторону границы лежит искомый индекс, и рекурсия идёт
**только туда**. Вторую половину не трогаем вообще.

```python
def findKthLargest(self, nums, k):
    target = len(nums) - k          # k-й по УБЫВАНИЮ = target-й по ВОЗРАСТАНИЮ

    def quick_select(left, right):
        if left == right:           # остался один элемент — это ответ
            return nums[left]

        pivot_idx = self.partition(nums, left, right)

        if target <= pivot_idx:
            return quick_select(left, pivot_idx)        # ищем слева от границы
        else:
            return quick_select(pivot_idx + 1, right)   # ищем справа

    return quick_select(0, len(nums) - 1)
```

**Откуда `target = len(nums) - k`.** Массив сортируется по возрастанию, а нужен
`k`-й по убыванию. Для `nums = [3,2,1,5,6,4]`, `k = 2`:

```
отсортированный:  [1, 2, 3, 4, 5, 6]
индексы:           0  1  2  3  4  5
2-й по убыванию — это 5, его индекс = 6 - 2 = 4 ✅
```

Проверка на краях: `k = 1` (максимум) → `target = n - 1`, последний индекс; `k = n`
(минимум) → `target = 0`. Сходится.

### Почему `O(n)` в среднем

Quick sort обрабатывает **обе** половины, quickselect — **одну**:

```
quick sort:    n + (n/2 + n/2) + (n/4 × 4) + ...  = n на каждом уровне  → n log n
quickselect:   n + n/2 + n/4 + n/8 + ...          = 2n                  → O(n)
```

Сумма геометрической прогрессии со знаменателем ½ равна `2n` — то есть линейно, без
логарифмического множителя.

| | Оценка |
|---|---|
| Время (средний) | `O(n)` |
| Время (худший) | `O(n²)` — лечится случайным пивотом |
| Память | `O(1)` на данные, `O(log n)` на стек |

### Две схемы разбиения

В папке лежат обе, отдельными файлами-шпаргалками:

| | Схема Ломуто (`lomuto_scheme_devision.py`) | Схема Хоара (`hoar_scheme_partition.py`) |
|---|---|---|
| Что возвращает | **точный финальный индекс** пивота | **границу** между зонами (пивот на месте не стоит) |
| Инвариант | `< pivot` слева, `>= pivot` справа | всё слева `<=` всего справа |
| Указатели | один бегущий + `store_idx` | встречные с двух концов |
| Обменов | больше | примерно втрое меньше |
| На дубликатах | **деградирует** | держится |

**Почему решение с Ломуто получило TLE.** В схеме Ломуто условие обмена —
`nums[i] < pivot`. Если все элементы равны, оно не выполняется **ни разу**,
`store_idx` остаётся на `left`, и разбиение выходит вырожденным: 0 элементов слева
и `n-1` справа. Каждый следующий вызов уменьшает диапазон на единицу — `O(n²)`.
Тест `test_lomuto_all_equal_gives_worst_split` фиксирует именно это поведение.

Схема Хоара на том же входе останавливает **оба** указателя на каждом равном
элементе (`< pivot` и `> pivot` — оба строгие) и продолжает меняться местами,
поэтому граница уезжает примерно в середину — деление честно пополам.

### Схема Хоара по шагам

```python
def partition(self, nums, left, right):
    pivot = nums[random.randint(left, right)]

    i = left - 1                  # указатели стартуют ЗА границами диапазона:
    j = right + 1                 # в цикле сначала сдвиг, потом проверка (do-while)

    while True:
        i += 1
        while nums[i] < pivot:    # остановится на первом элементе >= pivot
            i += 1

        j -= 1
        while nums[j] > pivot:    # остановится на первом элементе <= pivot
            j -= 1

        if i >= j:                # указатели встретились — вся область проверена
            return j

        nums[i], nums[j] = nums[j], nums[i]   # оба стоят не в своей половине
```

**Почему `i = left - 1`, а не `left`.** Внутренний цикл начинается со сдвига, то
есть эмулирует `do-while`. Без стартового сдвига наружу первый элемент диапазона
был бы пропущен, а указатель мог бы застрять на месте и зациклиться.

**Почему `<` и `>` строгие.** Именно это заставляет оба указателя останавливаться
на элементах, равных пивоту, и обменивать их. Обмены выглядят бессмысленными
(меняем равное на равное), но за счёт них граница уезжает в середину — это и есть
защита от массива из одинаковых значений.

> ⚠️ **Тонкость выбора пивота в схеме Хоара.** Классическая гарантия «возвращаемая
> граница строго меньше `right`» выполняется, только если пивот берётся из
> `[left, right - 1]`. При `random.randint(left, right)` есть редкий случай: если
> пивотом выбран **строгий максимум, стоящий последним**, левый указатель проходит
> весь диапазон и функция возвращает `j == right`.
>
> Для 215 это не ломает ответ — `quick_select` просто вызовется на том же диапазоне
> и возьмёт новый случайный пивот, так что алгоритм сходится (вероятность случая —
> `1/m` на вызов). Но подмассив на этом шаге не уменьшается, и формальная гарантия
> сходимости теряется. Надёжнее `random.randint(left, right - 1)` — тогда пивот
> заведомо не является строгим максимумом хвоста. Тесты
> `test_hoare_boundary_can_reach_right` и `test_hoare_boundary_shrinks_with_inner_pivot`
> фиксируют оба поведения.

### Альтернативы для 215

| Подход | Время | Память | Когда лучше |
|---|---|---|---|
| **Quickselect** (в файле) | `O(n)` в среднем | `O(1)` | общий случай |
| Куча размера `k` (`heapq.nlargest`) | `O(n log k)` | `O(k)` | `k ≪ n`, поток данных, нельзя менять массив |
| Полная сортировка `sorted(nums)[-k]` | `O(n log n)` | `O(n)` | одна строка, когда производительность не критична |
| Counting sort по значениям | `O(n + k)` | `O(k)` | здесь применимо: по условию `-10⁴ <= nums[i] <= 10⁴` |

> 📎 Тот же приём «нужны не все элементы, а только k» встречается в 347 (§12,
> решено через bucket sort) и 973 (K Closest Points to Origin).
...
показывает приём, который дальше переиспользуется в поразрядной сортировке (§11).
...
Ровно эта процедура и является одним проходом radix sort (§11).
...
## 11. Поразрядная сортировка (Radix Sort)
...
## 12. Блочная сортировка (Bucket Sort)
...
## 13. Timsort — то, что внутри `sorted()`

Файлы: `e451_Sort_Characters_By_Frequency_Bucket_sort.py` (метод `TimSort`),
`e2418_Sort_the_People.py` (метод `myFirstSolution`)
...
| [148. Sort List](https://leetcode.com/problems/sort-list/) | `e148_Sort_List.py` | merge sort на связном списке | `O(n log n)` | `O(log n)` |
| [164. Maximum Gap](https://leetcode.com/problems/maximum-gap/) | `e164_Maximum_Gap_Radix_Sort.py` | radix sort (LSD) | `O(d·(n + 10))` | `O(n)` |
| [215. Kth Largest Element in an Array](https://leetcode.com/problems/kth-largest-element-in-an-array/) | `e215_Kth_Largest_Element_in_an_Array.py` | quickselect (схема Хоара) | `O(n)` в среднем | `O(log n)` |
| [2418. Sort the People](https://leetcode.com/problems/sort-the-people/) | `e2418_Sort_the_People.py` | selection sort + Timsort | `O(n²)` / `O(n log n)` | `O(n)` |
...
| [912. Sort an Array](https://leetcode.com/problems/sort-an-array/) | `e912_Bubble_Sort_an_Array.py` | quick sort (рабочий) + heap sort + bubble sort (учебный) | `O(n log n)` / `O(n²)` | `O(log n)` |

Отдельно лежат две шпаргалки по схемам разбиения — они не решают задачу, а
разбирают процедуру, на которой стоят quick sort и quickselect:

| Файл | Что внутри |
|---|---|
| `hoar_scheme_partition.py` | схема Хоара: встречные указатели, возвращает границу зон |
| `lomuto_scheme_devision.py` | схема Ломуто: один бегущий указатель, возвращает точный индекс пивота |

---

## 15. Покрытие видов сортировок
...
| Сортировка выбором (Selection) | ✅ | 2418 |
| Сортировка подсчётом (Counting) | ✅ | 75 |
| Блочная (Bucket) | ✅ | 347, 451 |
| Поразрядная (Radix) | ✅ | 164 |
| Пирамидальная (Heap) | ✅ | 912 |
| Timsort | ✅ | 451, 2418 |
| Сортировка слиянием (Merge) | ✅ | 148 |
| Быстрая сортировка (Quick) | ✅ | 912 |
| Quickselect (k-й элемент) | ✅ | 215 |
...
| В selection sort обмен делают внутри внутреннего цикла | Получается пузырёк: `O(n²)` обменов вместо `O(n)` | Запоминать `max_idx`, менять **после** внутреннего цикла |
| В selection sort сортируют один массив из пары связанных | Имя и рост разъезжаются, ответ неверный | Менять оба массива в одном обмене (или `zip` в пары) |
| Внешний цикл selection sort до `n`, а не `n - 1` | Лишняя итерация впустую | Последний элемент встаёт на место сам |
...
| В quick sort пивот — всегда крайний элемент | `O(n²)` и `RecursionError` на отсортированном входе (TLE на 912) | Случайный пивот или медиана из трёх |
| В quick sort сохранили индекс пивота, а не значение | После обменов по этому индексу лежит уже другой элемент | Брать `pivot = nums[random.randint(...)]` — значение |
| Рекурсия в quick sort от `(low, right)` и `(right + 1, high)` | Для схемы Хоара границы другие — часть элементов теряется | `(low, right)` и `(left, high)` |
| В схеме Хоара нестрогие сравнения `<=` / `>=` | Указатели пробегают равные элементы насквозь → выход за границы массива | Только строгие `<` и `>` |
| В схеме Хоара указатели стартуют с `left` и `right` | Первый элемент диапазона пропускается, возможен вечный цикл | `i = left - 1`, `j = right + 1` (эмуляция do-while) |
| В схеме Хоара пивот берут из `[left, right]` | Возможен возврат `j == right` — подмассив не уменьшается | Брать пивот из `[left, right - 1]` |
| Quickselect на дубликатах через схему Ломуто | Разбиение 0 / n-1 на равных элементах → `O(n²)`, TLE на 215 | Схема Хоара либо трёхпутевое разбиение |
| В quickselect рекурсия идёт в обе половины | Теряется весь выигрыш: `O(n log n)` вместо `O(n)` | Спускаться только в ту часть, где лежит `target` |
...
| В merge sort `fast` стартует с `head`, а не с `head.next` | На списке из 2 элементов левая часть = весь список → бесконечная рекурсия | `fast = head.next`, `slow = head` |
| Забыли `slow.next = None` после поиска середины | Половины остаются одним списком, рекурсия не доходит до базы | Разрывать список сразу после нахождения `mid` |
| В merge остаток второго списка дописывают поэлементно | Лишний проход (на корректность не влияет) | `current.next = idx2 if idx1 is None else idx1` |
| В merge условие `<` вместо `<=` | Сортировка теряет стабильность — при равенстве берётся более поздний элемент | `if idx1.val <= idx2.val` |

---

## 17. Как выбрать сортировку
...
flowchart TD
    ...
    D -->|"Нужен только k-й элемент,<br/>а не весь порядок"| J["Quickselect, O(n)"]
```
...
**Что учить дальше.** Все основные виды сортировок в папке разобраны (см. §15).
Дальше — смежные темы:
...
| Трёхпутевое разбиение | [75. Sort Colors](https://leetcode.com/problems/sort-colors/) | голландский флаг как partition для quick sort на дубликатах |
| Сортировка на связном списке | [23. Merge k Sorted Lists](https://leetcode.com/problems/merge-k-sorted-lists/) | слияние `k` списков кучей — развитие 148 |
| Внешняя сортировка | — | что делать, когда данные не помещаются в память |
