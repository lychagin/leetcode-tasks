...
Ref: https://leetcode.com/problems/intersection-of-two-linked-lists/description/?envType=problem-list-v2&envId=linked-list
...
class Solution:
    def getIntersectionNode(self, headA: ListNode, headB: ListNode) -> Optional[ListNode]:
        ...
        while lineA != lineB:
            #print(f"A: {lineA.val}; B: {lineB.val}")
...
def getListEnd(head: ListNode) -> ListNode:
    prev = None
    while head:
        prev = head
        head = head.next
...
listA = makeList([4, 1])
endA = getListEnd(listA)
listB = makeList([5, 6, 1])
endB = getListEnd(listB)
listC = makeList([8, 4, 5])
endA.next = listC
endB.next = listC
